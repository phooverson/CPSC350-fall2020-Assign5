...
void ManageDataBase::PromptUser(){ //acts as a control base for all functions
  updateBSTsFromFile();
  ...
  int optionNumber;
  while (optionNumber != 14){ //runs until the user is done
...
void ManageDataBase::updateBSTsFromFile(){ //updates all BST's from a file
  updateFacultyBST();
  updateStudentBST();
}

void ManageDataBase::updateFacultyBST(){ //updates faculty BST
  string line;
...
void ManageDataBase::updateStudentBST(){ //updates student BST
  string line;
...
void ManageDataBase::CreateStudent(){ //create a student and add them to the BST
  int id;
  string level;
  string major;
  double gpa;
  ...
  if (!facultyTree->searchNodeById(advisorId)){ // Check to see if the advisor exists.  If not then prompt to create an advisor.
    cout << "The advisor does not exist.  Please create the advisor as a faculty member now." << endl;
    CreateFacultyMember();
  }
...
void ManageDataBase::DeleteStudent(){ //remove student from BST
  int studentId;
...
void ManageDataBase::PrintStudent(){ //print a student based off of an id
  // Initialize values
  int id;
...
void ManageDataBase::PrintFacultyMember(bool onlyAdvisees){ //print a faculty member based of of an id
  // Initialize values
  int id;
...
void ManageDataBase::ChangeAdvisor(){ //change the advisor of a student
  // Initialize Values
  int studentId;
  int advisorId;
...
void ManageDataBase::CreateFacultyMember(){ //create a faculty member and add to the BST
...
void ManageDataBase::DeleteFaculty(){ //delete a facultly memeber from the BST
  int facultyId;
  int currStudentId;
  ...
  if (facultyTree->searchNodeById(facultyId)){
    ...
    while (!adviseesToBeReassigned->isEmpty()){
      ...
      if (studentTree->searchNodeById(currStudentId)){
        ...
        // ADD THE ADVISEE TO THE FACULTY MEMBER NOW
      }
    }
...
void ManageDataBase::PrintAdvisor(){ //print advisor of a student
  // Initialize values
  int studentId;
  int advisorId;
...
void ManageDataBase::RemoveAdvisee() { //remove advisee from a facutly member
  // Initialize values
  int studentId;
  int advisorId;
  int newAdvisorId;
  ...
  // Redefine student advisor.
}

void ManageDataBase::insertOperationType(int num){ //add a integer, coresponding to faculty or student, to a stack for rollback purposes
  lastOperation->push(num);//1 = student 2 = faculty
}
...
void ManageDataBase::Rollback(){ //undo the last creation or deletion of a faculty/student
  ...
  if (lastOperationNum == 1){ //student
    ...
    if (studentTree->searchNodeById(student->GetId())) {
      if (studentTree->deleteNode(student))
        cout << "Removing student from tree." << endl;//stuent deleted
    }
    else {
